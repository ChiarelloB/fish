...
const Shop: React.FC = () => {
  const { algas, updateAlgas, inventory } = useGame();
  ...
  const shopItems: Record<string, ShopItem[]> = {
    ...
    eggs: [
      { id: 'egg-1', name: 'Ovo Comum', type: 'egg', price: 200, image: '🥚', description: 'Ovo básico - Maior chance de peixes comuns', rarity: 'Comum' },
      { id: 'egg-2', name: 'Ovo Raro', type: 'egg', price: 800, image: '🔮', description: 'Ovo raro - Boa chance de peixes raros', rarity: 'Raro' },
      { id: 'egg-3', name: 'Ovo Épico', type: 'egg', price: 2500, image: '💎', description: 'Ovo épico - Alta chance de peixes épicos', rarity: 'Épico' },
      { id: 'egg-4', name: 'Ovo Lendário', type: 'egg', price: 8000, image: '👑', description: 'Ovo lendário - Chance de peixes lendários', rarity: 'Lendário' },
      { id: 'egg-5', name: 'Ovo Transcendente', type: 'egg', price: 25000, image: '🌌', description: 'Ovo transcendente - Peixes ultra-raros', rarity: 'Transcendente' }
    ]
  };
  ...
  const handlePurchase = async (item: ShopItem) => {
    ...
    try {
      ...
      // Simular adição ao inventário (seria salvo no Firebase)
          ...
          {shopItems[activeTab]?.map((item, index) => (
            ...
            >
              <div className="text-center mb-4">
                ...
                {item.rarity && (
                  <div className={`inline-block px-3 py-1 rounded-full text-white text-xs font-medium ${getRarityColor(item.rarity)} mb-3`}>
                    {item.rarity}
                  </div>
                )}
